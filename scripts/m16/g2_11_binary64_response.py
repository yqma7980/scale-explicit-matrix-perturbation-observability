...
def direct_route(output_root: Path) -> bool:
    ...
    with np.load(SNAPSHOT, allow_pickle=False) as data:
        ...
        rhs = np.asarray(data["rhs_hat"], dtype=np.float64)
    z_phys = scipy.linalg.solve(
        jhat_phys, rhs, assume_a="gen", overwrite_a=False,
        overwrite_b=False, check_finite=True,
    )
    z_reg = scipy.linalg.solve(
        jhat_reg, rhs, assume_a="gen", overwrite_a=False,
        overwrite_b=False, check_finite=True,
    )
    ...
    np.savez_compressed(
        output_root / "g2_11_direct_route_vector.npz",
    ...
    write_json(output_root / "g2_11_binary64_direct_summary.json", result)
...
def response_route(output_root: Path) -> bool:
    ...
    with np.load(SNAPSHOT, allow_pickle=False) as data:
        ...
        jhat_reg = np.asarray(data["jhat_reg"], dtype=np.float64)
    with np.load(
        output_root / "g2_11_direct_route_vector.npz", allow_pickle=False
    ) as data:
        z_phys = np.asarray(data["z_phys64"], dtype=np.float64)
    ...
    np.savez_compressed(
        output_root / "g2_11_response_route_vector.npz",
        delta_j64=delta_j,
        rhs_response64=rhs,
    ...
    write_json(output_root / "g2_11_binary64_response_summary.json", result)
    return bool(
        result["finite"]
        and result["delta_j_nonzero_count"] == 900
        and result["delta_j_off_diagonal_nonzero_count"] == 0
        and result["delta_j_diagonal_nonzero_count"] == 900
