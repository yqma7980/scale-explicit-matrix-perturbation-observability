...
SNAPSHOT = RELEASE_ROOT / "data/M16_operands_binary64.npz"
EVAL_DPS = 180
...
def reference_audit(ref120_path: Path, ref180_path: Path, output_root: Path) -> bool:
    ...
    write_json(
        output_root / "g2_11_reference_precision_audit.json",
        {
            "schema_version": "1.0",
    ...
    write_json(
        output_root / "g2_11_parent_reference_metrics.json",
        {
            "schema_version": "1.0",
    ...
    write_json(
        output_root / "g2_11_response_identity_audit.json",
        {
            "schema_version": "1.0",
    ...
    write_json(
        output_root / "g2_11_inverse_norm_metrics.json",
        {
            "schema_version": "1.0",
...
def direct_uncertainty(ref180_path: Path, output_root: Path) -> bool:
    ...
    with np.load(output_root / "g2_11_direct_route_vector.npz", allow_pickle=False) as data:
        delta_direct = mp_vector(data["delta_direct64"])
        z_phys = mp_vector(data["z_phys64"])
        z_reg = mp_vector(data["z_reg64"])
    ...
    u_sub = gamma1 * norm2(absolute_parent_sum) + mp.sqrt(900) * mp.power(2, -1075)
    ...
    for component, space, actual, bound in [
        ...
        rows.append(row)
    write_csv(output_root / "g2_11_direct_uncertainty_ledger.csv", rows)
    write_json(
        output_root / "g2_11_direct_uncertainty_summary.json",
        {
            "schema_version": "1.0",
            "U_z": decimal(u_z),
            "U_zg": decimal(u_zg),
...
def response_uncertainty(ref180_path: Path, output_root: Path) -> bool:
    ...
    with np.load(output_root / "g2_11_direct_route_vector.npz", allow_pickle=False) as data:
        z_phys = mp_vector(data["z_phys64"])
    with np.load(output_root / "g2_11_response_route_vector.npz", allow_pickle=False) as data:
    ...
    direct_summary = json.loads(
        (output_root / "g2_11_direct_uncertainty_summary.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    for component, space, actual, bound in [
        ...
        row.update(tightness(actual, bound))
        rows.append(row)
    write_csv(output_root / "g2_11_response_uncertainty_ledger.csv", rows)
    write_json(
        output_root / "g2_11_response_uncertainty_summary.json",
        {
            "schema_version": "1.0",
...
def finalize(output_root: Path, cost_path: Path | None) -> bool:
    ...
    response_summary = json.loads(
        (output_root / "g2_11_response_uncertainty_summary.json").read_text(
            encoding="utf-8"
        )
    )
    ...
    write_json(
        output_root / "g2_11_route_comparison.json",
        {
            "schema_version": "1.0",
    ...
    write_json(
        output_root / "g2_11_distinguishability_decisions.json",
        {
            "schema_version": "1.0",
    ...
    direct_rows = list(
        csv.DictReader(
            (output_root / "g2_11_direct_uncertainty_ledger.csv").open(
    ...
    response_rows = list(
        csv.DictReader(
            (output_root / "g2_11_response_uncertainty_ledger.csv").open(
                "r", encoding="utf-8", newline=""
            )
        )
    )
    ...
    write_json(
        output_root / "g2_11_tightness_and_cost.json",
        {
            "schema_version": "1.0",
